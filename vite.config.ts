...
export default defineConfig(({ command, mode }) => {
  if (mode !== 'build') plugins.push(vitePluginLangspack({ lang: { src: pathResolve('langs'), dst: pathResolve('public'), include: /.json$/ }, log: { level: 6, filters: ['/node_modules/'], maxLen: 200 } }));
